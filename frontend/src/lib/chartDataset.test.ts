...
/**
 * El constructor genérico pinta columnas ARBITRARIAS (incluidas las tablas que
 * devuelve Moby), así que un negativo es un valor legítimo — una delta, una
 * diferencia — y tiene que llegar al gráfico. Es la diferencia con
 * `toMetricValue`, que sí anula los negativos porque sus cuatro vistas sólo
 * miran recuentos de pacientes.
 */
const deltaRows: DataRow[] = [
  { account_id: "d1", account_name: "Madrid",    country: "ES", data: { delta: -12 } },
  { account_id: "d2", account_name: "Barcelona", country: "ES", data: { delta: 30 } },
  { account_id: "d3", account_name: "Milano",    country: "IT", data: { delta: 0 } },
  { account_id: "d4", account_name: "Roma",      country: "IT", data: {} },
];
...
describe("buildChartDataset — fila a fila", () => {
  ...
  it("conserva el negativo: en una columna arbitraria (una delta) es un valor, no basura", () => {
    const out = buildChartDataset(deltaRows, NAME, ["delta"]);
    expect(out.map((r) => r.delta)).toEqual([-12, 30, 0, null]);
  });

  it("no confunde el negativo con el ausente: el que no reporta sigue en null", () => {
    const out = buildChartDataset(deltaRows, NAME, ["delta"]);
    expect(out[3].delta).toBeNull();
  });
...
describe("buildChartDataset — agrupado por país", () => {
  ...
  it("suma los negativos en vez de tirarlos: la delta del grupo puede quedar negativa", () => {
    const out = buildChartDataset(deltaRows, "country", ["delta"]);
    expect(out.find((r) => r.country === "ES")?.delta).toBe(18); // -12 + 30
    expect(out.find((r) => r.country === "IT")?.delta).toBe(0);  // 0 reportado + Roma ausente
  });
});
...
describe("toPieSlices", () => {
  ...
  // El pie es la única vista donde el negativo NO se puede pintar: una porción de
  // ángulo negativo no es geometría. Se excluye, pero se CUENTA para que la UI lo
  // diga en voz alta — que es justo lo contrario de tirarla en silencio.
  it("excluye la porción negativa y la cuenta, en vez de tirarla en silencio", () => {
    const withDelta = [
      { [NAME_KEY]: "Madrid", delta: -12 },
      { [NAME_KEY]: "Milano", delta: 30 },
    ];
    const { slices, negativeExcluded } = toPieSlices(withDelta, NAME_KEY, ["delta"]);
    expect(slices.map((s) => s[NAME_KEY])).toEqual(["Milano"]);
    expect(negativeExcluded).toBe(1);
  });

  it("no cuenta como negativo al ausente: son cosas distintas", () => {
    const { negativeExcluded } = toPieSlices(pieRows, NAME_KEY, ["a"]);
    expect(negativeExcluded).toBe(0);
  });
});
