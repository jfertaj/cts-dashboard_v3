...
from backend.app.routers.moby_tools import (
    TOOL_DISPATCH,
    ToolContext,
    ToolResult,
    dispatch_tool,
...
def test_sql_query_handler_error_path():
    ...
    with patch("backend.app.routers.moby_tools.handle_sql_query.__module__"):
...
def test_explorer_set_filters_calls_tool():
    ...
    with patch("backend.app.routers.ai_chat.tool_explorer_set_filters", return_value={"ok": True}):
...
def test_manipulate_data_preserves_state():
    ...
    with patch("backend.app.routers.ai_chat.tool_manipulate_data", return_value={"rows": [{"a": 1}]}):
...
def test_render_chart_uses_last_table_when_no_data():
    ...
    with patch("backend.app.routers.ai_chat.tool_render_chart", return_value={"visualization": fake_viz}):
        with patch("backend.app.routers.ai_chat._pretty_label", side_effect=lambda x: x):
...
def test_explorer_within_drive_km_no_base_id():
    ...
    with patch("backend.app.routers.ai_chat._first_account_id_from_table", return_value=None):
