...
**Gotcha/Patrón**:
1. **Los dos extremos del contrato de columnas discrepaban, y el modo de fallo era `null`, no un error.** El frontend manda las columnas SIN el prefijo (`lib/api.ts`: `sf.Foo` → `Foo`) y el constructor del SELECT sólo aceptaba las que empezaban por `sf.` — o sea que se descartaban TODAS. Al SOQL sólo llegaban los 8 campos sembrados a mano y los que tocara un filtro activo; el resto de las ~360 claves volvía a `null` porque el data loop hacía `o.get(k)` sobre una `o` que jamás los había seleccionado. **Cuando los dos lados de un contrato de serialización se escriben por separado, el bug no falla: devuelve vacío.** Una discrepancia de prefijo entre cliente y servidor no lanza excepción — hay que probarla con una aserción de VALOR, no de forma.
2. **Arreglarlo en el frontend (dejar de strippear) cambia un bug silencioso por otro.** El strip es load-bearing para la familia `Account.*`: `sf.Account.Name` → `Account.Name` es justo lo que hace que caiga en la rama de `acc_map`. Sin strip caería en la rama `sf.` con `fld="Account.Name"`, que falla los `_exists_on_*` (contienen `Name`, no `Account.Name`) → `None` otra vez. **El sitio correcto para tolerar las dos formas es el backend**, que es quien conoce el esquema.
3. **`_exists_on_opportunity()` NO basta como gate para una clave pelada: en modo permisivo devuelve `True` a TODO.** (`_DESCRIBE_PERMISSIVE` se activa si el `describe()` de SF falla al arrancar.) Una clave prefijada basura ya tenía ese riesgo, pero con las peladas el espacio se abre a columnas virtuales del frontend como `MemberName` (`sf.MemberName`, api.ts:325), que no es un campo de Opportunity: si entra en el SELECT, el SOQL entero revienta con `INVALID_FIELD` y la búsqueda pasa de "devuelve nulls" a "no devuelve nada". Por eso la forma pelada se valida contra `ALLOWED_FIELDS` (el catálogo curado que alimenta `/api/explorer/fields`, o sea exactamente lo que el frontend puede pedir). **Un fallback permisivo pensado para no bloquear al usuario se convierte en un agujero en cuanto lo usas como validador de entrada.**
4. **Un `git checkout -- <fichero>` para revertir un mutante se lleva por delante TODO el trabajo sin commitear de ese fichero.** Me pasó a mitad del fix: perdí las 5 ediciones. Para un mutation check: `cp` el fichero al scratchpad, mutar, testear, y restaurar con `cp` — nunca con `git checkout`, salvo que lo que quieras tirar sea justamente todo.

**Por qué importa**: el fill perezoso (`lib/fillColumns.ts`) tapaba el agujero sólo para las columnas que alguien se acordara de registrar. La causa raíz dejaba armada la trampa para toda feature futura que leyera `row.data`. Y (3) es lo que impide que el fix cambie un null silencioso por una caída dura del endpoint cuando SF se pone tonto al arrancar.

**Dónde aplicar**: 1-3 → `backend/app/routers/salesforce_explorer.py` y cualquier endpoint que reciba claves de columna del frontend. Los otros 3 constructores de SELECT del fichero (`/columns/fill` y los dos de nearby) siguen exigiendo `sf.` **correctamente**: `api.ts:347` es el ÚNICO sitio que strippea, así que esos reciben la forma prefijada. 4 → todo mutation check de este repo.
...
### [2026-07-14] [coder] — Un embudo en eje absoluto no es un embudo: la escala tiene que ser la RETENCIÓN (y en SVG `allInnerTexts()` devuelve `undefined`)

**Contexto**: `frontend/src/components/charts/FunnelView.tsx` — con los datos de producción (215 centros, 73 completos) el embudo sumaba ~22.000 cribados → unos cientos en Stage 1 → menos en Stage 2. Tres barras sobre un eje lineal compartido: la primera ERA el gráfico y las otras dos, slivers pegados a la base. El dato era CORRECTO; el gráfico era inútil, porque la relación Stage 1 → Stage 2 — la única pregunta que un embudo existe para responder — no se podía leer.

**Gotcha/Patrón**: en una caída de dos órdenes de magnitud, **ninguna escala compartida de absolutos puede mostrar la proporción**. El eje log la haría visible pero convierte una razón de 43× en "un poco más corta": engaña al ojo justo en la dimensión que el usuario está leyendo. La salida es cambiar **qué codifica la longitud de la barra**: no el absoluto, sino el **% que sobrevive de la etapa anterior** (100 % → 2,3 % → 74 %). El absoluto no se pierde — va en la etiqueta pegada a la barra y en una tira de tarjetas de texto, que es lo único legible cuando la barra es un sliver de un píxel. Reglas de la aritmética (`toFunnelSteps`, exportada y con tests): la etapa anterior a 0 da `retentionPct = null`, **nunca** `NaN` (0/0) ni `Infinity` (n/0), y `null` se dice con palabras ("sin base"), no disfrazado de `0 %` — un 0 % medido y una división imposible no son lo mismo, igual que reportar-cero y no-reportar. Y el eje no se recorta a 100: una etapa que reporta MÁS que la anterior (dato corrupto pero real) tiene que desbordar y verse.

**Cómo se testea**: contar `<g>` no prueba nada (ya sabido). Aquí el test que separa los dos diseños es una **razón de anchos**: con retención la 3.ª barra mide 0,40 de la 1.ª; con el eje absoluto viejo, 0,20. Mutación verificada (`barPct: stage.value` → CHART-12 rojo con `Received: 0.199…`). **Trampa nueva**: las etiquetas de Recharts son `<text>` SVG y `SVGElement` **no tiene `innerText`** — `locator.allInnerTexts()` devuelve `[undefined, undefined, …]` sin fallar en el locator. Hay que usar `allTextContents()`. Y el E2E corre contra `dist` precompilado: sin `npm run build` se valida el bundle viejo.

**Por qué importa**: el gráfico anterior no mentía en los números pero sí en la lectura — un usuario no podía ver dónde se cae la gente, que es para lo que existe la pestaña. Y un `NaN`/`Infinity` colado en `barPct` habría reventado el eje entero de Recharts.

**Dónde aplicar**: `FunnelView.tsx`. La regla general — "si el rango dinámico mata la proporción, cambia lo que codifica la longitud, no la escala del eje" — vale para cualquier gráfico nuevo de este modal. El gotcha de `allTextContents()` vale para **todo** E2E que lea texto de un gráfico (etiquetas, ejes, leyendas de Recharts son SVG).

### [2026-07-14] [coder] — "Ask Moby" era la superficie que se quedó atrás del fix de `chartRows`: mandaba el array de respaldo, no lo que la tabla enseña

**Contexto**: `frontend/src/pages/ExplorerView.tsx`, botón "Ask Moby" (~línea 3086). Mismo bug de clase que CHART-8/9 (ver entrada anterior "Los gráficos agregan las filas filtradas..."), pero en la superficie que ese fix no tocó: el handler de "Ask Moby" seguía leyendo `const activeRows = nearbyActive ? fullNearbyRows : fullRows` — el array de respaldo SIN los filtros de cliente (búsqueda global + filtros por columna) — y lo escribía en `sessionStorage["moby_last_table_v1"]` + `cts:explorer:ask-ai`. Commit `8d89fd8`.

**Gotcha/Patrón**: cuando una fuente de verdad (`table.getFilteredRowModel()`) se establece para UNA superficie (gráficos), hay que **grepear todos los demás consumidores del array de respaldo viejo** (`fullRows`/`fullNearbyRows`) en el mismo fichero — no asumir que el fix de una superficie cubre a las demás. El fix aquí fue reusar el memo `chartRows` ya existente (no rederivar): una sustitución de 2 líneas. También hasta ahora nadie comprobaba que el `rowCount` del evento coincidiera con `rows.length` del payload — son la misma mentira en miniatura si divergen (p.ej. si uno usa `.length` sin cap y el otro un array `.slice(0,500)`; aquí es correcto a propósito: `rowCount = chartRows.length` sin cap — es la población real, aunque `rows` se recorte a 500 — igual que el badge del botón Chart).

**Por qué importa**: el usuario acota la tabla a 12 sitios visibles y pregunta "cuántos hay en España" — Moby contestaba sobre los 215 originales, incluidos los 203 que el usuario acaba de sacar de la vista y no puede ver, con toda confianza.

**Dónde aplicar**: todo grep futuro por `fullRows`/`fullNearbyRows` en `ExplorerView.tsx` debería preguntarse si ese consumidor necesita el filtrado de cliente (`chartRows`) o de verdad necesita el respaldo sin filtrar (p.ej. el nearby-panel que gestiona su propia selección). No asumir automáticamente.

### [2026-07-14] [coder] — La UI va en INGLÉS y los comentarios en ESPAÑOL: son dos convenciones distintas, y el `toLocaleString` sin locale te delata

**Contexto**: `frontend/src/components/charts/*` + `lib/chartAggregation.ts` — todo el rediseño del modal de gráficos se implementó con strings de UI en español (pestañas, cobertura, estados vacíos, etapas del embudo). La app entera es inglesa ("Account Name", "Rows per page", "Export filtered (TSV)"). Commit de arreglo: `5731407`.
**Gotcha/Patrón**: (1) La voz española del repo vive en los COMENTARIOS, nunca en los strings que ve el usuario — si un brief dice "labels en español", está mal, comprueba la UI existente antes de escribir el primer literal. (2) `value.toLocaleString()` sin locale usa el del NAVEGADOR: con Chrome en español escribe `74,2` y `22.000` dentro de una UI inglesa, donde la coma se lee como millar. Los números de cobertura/conversión dirían cosas distintas según quién los mire. Fija `"en-US"` siempre. (3) Los strings de dominio (`"Cribados"`, `"(sin país)"`) viven en `lib/chartAggregation.ts`, no sólo en los componentes: pasan al eje Y y al tooltip vía `stage`/`country`. Traducir sólo los `.tsx` deja la mitad del embudo en español.
**Por qué importa**: un usuario clínico que ve "74,2 %" en una UI inglesa lee setecientos cuarenta y dos, y la línea de cobertura es justamente lo que este rediseño existe para que no mienta. Y "1 sites" en esa misma línea le resta credibilidad al número que va al lado (de ahí `lib/chartText.ts#siteCount`).
**Dónde aplicar**: todo el frontend. Cualquier string nuevo que renderice un componente → inglés; cualquier `toLocaleString`/`Intl` → locale explícito.

### [2026-07-14] [coder] — `ALLOWED_FIELDS` es un catálogo de COLUMNAS, no de campos de Opportunity: usarlo como gate del SELECT cuela `Assignment.*` y tumba el Explorer

**Contexto**: `backend/app/routers/salesforce_explorer.py#_column_key_to_sf_field` (commit a1bfeb2). El helper que resuelve una columna pedida al campo que va al `SELECT ... FROM Opportunity` validaba la clave PELADA contra `ALLOWED_FIELDS`, con un docstring que afirmaba que ese gate bastaba para frenar claves no-Opportunity en modo permisivo.

**Gotcha/Patrón**: no bastaba, y el fallo es de CATEGORÍA, no de caso. `ALLOWED_FIELDS` = `CURATED_ALLOWED | MIN_ALLOWED` = las 113 COLUMNAS que el frontend puede pedir a `/api/explorer/fields`, de las cuales **33 llevan punto**: 24 `Account.*` (que sí tapaba `_NON_OPP_COL_PREFIXES`) y **9 `Assignment.*`** (que NO tapaba nadie: `Assignment.Name`, `.Assignment_Type__c`, `.C_Account__c`, `.C_Assignment_Stage__c`, `.C_Contact_Name__c`, `.C_Invoice_Received__c`, `.C_MCA_Status__c`, `.C_Opportunity_Name__c`, `.C_Payment_Done__c`). Cadena completa: `/fields` sirve `sf.Assignment.Name` → el frontend manda TODAS las keys como `columns` → `api.ts:347` quita el `sf.` → llega `Assignment.Name` pelada → pasa el gate del catálogo → `_exists_on_opportunity()` dice `True` porque en **modo permisivo dice True a TODO** → `SELECT ..., Assignment.Name, ... FROM Opportunity` → `INVALID_FIELD` → **500 en todas las búsquedas**. Y el permisivo es **STICKY**: `_ensure_describes()` pone `_OPP_FIELD_SET = set()` + `_DESCRIBE_PERMISSIVE = True` ante un describe fallido y NO reintenta en toda la vida del proceso — un hipo transitorio de SF al arrancar una task de Fargate lo arma para siempre.

Regla que lo cierra: **una clave PELADA con `.` es un path de relación (`Account.X`, `Assignment.X`, `qual.x`, `extra.x`, `site.x`), jamás un campo plano de Opportunity → `None`**. Verificado que no rompe nada: las **80** claves peladas de `ALLOWED_FIELDS` son campos reales de Opportunity (cruce contra `config/sf_schema_full.json`: 0 ausencias) y ninguna lleva punto. La regla subsume `_NON_OPP_COL_PREFIXES` (sus 4 prefijos llevan punto), que se borró. La rama `sf.` se dejó intacta.

**Por qué importa**: (1) un allowlist sólo protege de lo que enumera — si el catálogo mezcla dos espacios de nombres (columnas de UI vs campos de un sObject), usarlo como gate de uno de ellos es un colador, y hay que buscar **la propiedad estructural** (aquí, el punto) en vez de parchear el caso que apareció. (2) Un test puede ser TEATRO: `test_bare_key_rejected_even_in_permissive_mode` usaba `"MemberName"`, que **no está en `ALLOWED_FIELDS`** — nunca ejercitó el gate que decía pinear y pasaba en verde contra el código roto. Un test de un gate DEBE usar una entrada que llegue al gate. (3) Un modo degradado que empeora bajo un cambio "de mejora" (antes: columnas a null; después: Explorer muerto) es peor que el bug original.

**Dónde aplicar**: todo `salesforce_explorer.py` y cualquier sitio que construya SOQL a partir de claves del frontend. Si mañana el catálogo curado gana un `Contact.*` o un `Member.*`, la regla del punto ya lo cubre sin tocar nada — y el test `test_every_dotted_allowed_field_is_rejected_as_bare_key` lo verifica en barrido (falla si el catálogo cambia de tamaño, a propósito).

### [2026-07-14] [coder] — Rellenar un 0 honesto en un COUNT() rompe en silencio el `is_empty` que lo buscaba: el arreglo y su compensación viven en ficheros distintos

**Contexto**: `backend/app/routers/salesforce_extras_batch.py#batch_fetch_account_extras` + `filter_engine._eval_extra_rule` (fix del review de Codex en el PR #12). El `extra.AssignmentsCount` se escribía sólo para las cuentas CON assignments (`for aid, cnt in counts_by_acc.items()`), así que una cuenta con cero se quedaba SIN la clave.

**Gotcha/Patrón**: hay DOS módulos que definen `batch_fetch_account_extras` — `salesforce_extras.py:258` (con un `defaults` que sí pone `AssignmentsCount: 0`) y `salesforce_extras_batch.py:16` (sin él). **El Explorer importa el SEGUNDO** (`salesforce_explorer.py:21`); el primero no lo importa nadie. Leer el módulo equivocado hizo adjudicar "el 0 ya llega, no lo toques" y dejar el bug vivo. Verifica SIEMPRE el `import` real antes de razonar sobre un helper con nombre duplicado.

Y lo importante: **arreglar el dato rompió el filtro que dependía del bug**. Moby traduce "sites not in any assignment" a `{field: extra.AssignmentsCount, operator: is_empty}` (`moby/prompt.py:239/407`), y `_eval_qual_rule` implementa `is_empty` como `v is None or v == ""` — con lo cual, en cuanto el backend manda un `0`, ese filtro deja de casar con **exactamente los sites que buscaba** y devuelve cero resultados. El fix del dato y la compensación del filtro son inseparables: `_eval_extra_rule()` trata las claves de `_COUNT_EXTRA_FIELDS` como vacías cuando el conteo es `0` **o** ausente, y delega todo lo demás en `_eval_qual_rule` (para cualquier otro campo, un `0` sigue siendo un valor y `is_empty` sigue siendo `False`).
